...
#            0 = VNF successfully created
    def createVNF(self):
        ...
        if not self.VNF_EXIST:
            call(['mkdir', './PyCOO/IMAGES/' + self.ID])
            call(['cp', './PyCOO/IMAGES/click-on-osv.qcow2', './PyCOO/IMAGES/' + self.ID + '/click-on-osv.qcow2'])
            self.VNF_EXIST = True
            self.applyVNF()
            return 0
        else:
...
#            0 = configuration successfully set
    def applyVNF(self):
        ...
        if not self.VNF_UP:
            call(['cp', './PyCOO/IMAGES/click-on-osv.xml', './PyCOO/IMAGES/' + self.ID + '/click-on-osv.xml'])

            configurationXML = ElementTree.parse('./PyCOO/IMAGES/' + self.ID + '/click-on-osv.xml')
            ...
            configurationXML.find('devices/disk/source').attrib['file'] = path.abspath('PyCOO/IMAGES/' + self.ID + '/click-on-osv.qcow2')
            ...
            configurationXML.write('./PyCOO/IMAGES/' + self.ID + '/click-on-osv.xml')
            return 0
        else:
            return -1
...
#        0 = VNF upped successfully
    def upVNF(self):
        ...
        if not self.VNF_UP:
            ...
            call(['virsh', 'create', path.abspath('./PyCOO/IMAGES/' + self.ID + '/click-on-osv.xml')], stdout = FNULL)
            self.VNF_UP = True
            return 0
        else:
            return -1
