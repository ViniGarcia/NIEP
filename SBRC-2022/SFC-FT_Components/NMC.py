...
class NET_MANAGER:
	...
	def __init__(self, default_net_ip, data_queue, data_mutex, data_semaphore, nsh_flag, consensus_elements_file = "~/Desktop/NIEP/SBRC-2022/SFC-FT_Consensus/ConsensusTest.csv"):
		...
		self.__connection_processes = self.__connection_manager.dict()
		
		self.__data_queue = data_queue
		self.__data_mutex = data_mutex
		self.__data_semaphore = data_semaphore

		self.__consensus_id = None #Ajustar aqui dps
		self.__consensus_elements = None #Ajustar aqui dps
		self.__consensus_processes = self.__connection_manager.dict()
	...
	def conRecvServer(self, consensus_connection, consensus_ip):
		...
		while True:
		 	...
		 	data_origin = str(consensus_msg[2]) + "." + str(consensus_msg[3]) + "." + str(consensus_msg[4]) + "." + str(consensus_msg[5])
		 	data_detination = str(consensus_msg[6]) + "." + str(consensus_msg[7]) + "." + str(consensus_msg[8]) + "." + str(consensus_msg[9])
		 	...
		 	if not data_key in self.__consensus_check:
		 		if self.__consensus_elements[consensus_ip][0] == 0:
		 			...
		 			for element in self.__consensus_elements:
			...
			#WEAK ACCEPT
		 		...
		 		if not self.__consensus_elements[consensus_ip][0] in self.__consensus_check[data_key][1]:
		 			self.__consensus_check[data_key] = (self.__consensus_check[data_key][0]+1, self.__consensus_check[data_key][0] + [self.__consensus_elements[consensus_ip][0]])
	...
	def conConnectionServer(self)
		...
		while True:
			consensus_connection, consensus_address = self.__default_net_socket.accept()
	...
	def conPropose(self, data_mark, data_origin_ip, data_destination_ip):
		...
		proposal_message = struct.pack(">H", data_mark, data_origin_ip, data_destination_ip)
		for address_fragment in data_origin_ip.split("."):
			proposal_message += bytes(address_fragment)
	...
	def requestConnections(self, servers_ip):
		...
		for ip in self.__consensus_elements:
			if self.__consensus_elements[ip][0] < self.__consensus_id:
				...
				try:
					...
					continue
				consensus_process = multiprocessing.Process(target = self.conRecvServer, kwargs = dict(consensus_connection=server_socket, consensus_ip=ip))
				consensus_process.start()
