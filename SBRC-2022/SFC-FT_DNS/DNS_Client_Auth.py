...
def query(dns_builder, std_private_key):
	...
	query_packet = query_packet + pkcs1_15.new(std_private_key).sign(SHA256.new(query_packet))
...
def server(recv_socket, dns_builder, fault_tolerance, std_public_key):
	...
	while True:
		...
		dns_response = dns_response[:-4] #O problema está aqui, o número de controle deve vir depois da assinatura!
		...
		if fault_tolerance == 0:
			signature = dns_response[-256:]
			message = dns_response[:-256]
			try:
				pkcs1_15.new(std_public_key).verify(SHA256.new(message), signature)
			except (ValueError, TypeError):
				continue
			
			print("\n", dns_builder.read_std_response(message), "\n")
			continue
...
#CRYPTO VARIABLES: used to encrypt the requests
std_public_key = None
std_private_key = None
...
if len(sys.argv) > 3:
	if os.path.isfile(sys.argv[1]):
		try:
			...
			std_private_key = RSA.importKey(key_file.read())
			key_file.close()
		except:
			print("ERROR: INVALID PRIVATE KEY FILE PROVIDED!")
			exit()
	else:
		print("ERROR: INVALID PRIVATE KEY FILE PATH PROVIDED! [EXPECTED: DNS-Client.py PRIVKEY_USER_FILE PUBKEY_SERVER_FILE FAULT_TOLERANCE SC_IP_1 SC_IP_2 .. SC_IP_N]")
		exit()

	if os.path.isfile(sys.argv[2]):
		try:
			key_file = open(sys.argv[2], "rb")
			std_public_key = RSA.importKey(key_file.read())
			key_file.close()
		except:
			print("ERROR: INVALID PUBLIC KEY FILE PROVIDED!")
			exit()
	else:
		print("ERROR: INVALID PUBLIC KEY FILE PATH PROVIDED! [EXPECTED: DNS-Client.py PRIVKEY_USER_FILE PUBKEY_SERVER_FILE FAULT_TOLERANCE SC_IP_1 SC_IP_2 .. SC_IP_N]")
		exit()

	try:
		fault_tolerance = int(sys.argv[3])
	except:
		print("ERROR: INVALID FAULT TOLERANCE ARGUMENT PROVIDED! [EXPECTED: DNS-Client.py PRIVKEY_USER_FILE PUBKEY_SERVER_FILE FAULT_TOLERANCE SC_IP_1 SC_IP_2 .. SC_IP_N]")
		exit()
else:
	print("ERROR: INVALID ARGUMENTS PROVIDED! [EXPECTED: DNS-Client.py PRIVKEY_USER_FILE PUBKEY_SERVER_FILE FAULT_TOLERANCE SC_IP_1 SC_IP_2 .. SC_IP_N]")
	exit()
...
setup("www.facebook.com", "192.168.123.4", "53", "192.168.122.1", "9999") #JUST FOR TESTS
...
server_process = multiprocessing.Process(target=server, kwargs=dict(recv_socket=resp_socket, dns_builder=std_dns_builder, fault_tolerance=fault_tolerance, std_public_key=std_public_key))
...
while True:
	...
	if action.startswith("send"):
		if std_target_name == None:
			...
			continue
		query(std_dns_builder, std_private_key)
