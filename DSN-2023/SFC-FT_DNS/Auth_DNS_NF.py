...
if len(sys.argv) == 3:
    nf_acc_address = sys.argv[2]
else:
    print("ERROR: INVALID ARGUMENTS PROVIDED! [EXPECTED: NF.py PVT_KEY_FILE IP_ADDRESS]")
    exit()
...
if os.path.isfile(sys.argv[1]):
    try:
        ...
        server_std_privkey = RSA.importKey(key_file.read())
        key_file.close()
    except Exception as e:
        print("ERROR: INVALID KEY FILE PROVIDED!")
        exit()
else:
    print("ERROR: INVALID KEY FILE PATH PROVIDED! [EXPECTED: NF.py PVT_KEY_FILE IP_ADDRESS]")
    exit()
        ...
        if len(ft_manager.getConnections()) > 3:
            ...
            if len(recv_request["queries"]) > 0:
                if recv_request["queries"][0][0] in dns_dictionary:
                    snd_response = dns_builder.create_std_response(recv_data[0][66:], 1, 1, 65000, dns_builder.create_std_response_data(1, (dns_dictionary[recv_request["queries"][0][0]],)), recv_request)
                    snd_address = (recv_data[3], struct.unpack(">H", recv_data[0][58:60])[0])
                    snd_response = snd_response + pkcs1_15.new(server_std_privkey).sign(SHA256.new(snd_response)) + struct.pack(">I", recv_data[2])
                    dns_socket.sendto(snd_response, snd_address)
                else:
                    dns_socket.sendto(recv_data[0][66:], ("8.8.8.8", 53))
                    snd_response = dns_socket.recvfrom(1024)
                    snd_address = (recv_data[3], struct.unpack(">H", recv_data[0][58:60])[0])
                    snd_response = snd_response + pkcs1_15.new(server_std_privkey).sign(SHA256.new(snd_response)) + struct.pack(">I", recv_data[2])
                    dns_socket.sendto(snd_response, snd_address)
            ...
            if client_control[recv_data[3]][recv_data[2]][index][1] >= majority_parameter:
                ...
                if len(recv_request["queries"]) > 0:
                    if recv_request["queries"][0][0] in dns_dictionary:
                        ...
                        snd_address = (recv_data[3], struct.unpack(">H", recv_data[0][58:60])[0])
                        snd_response = snd_response + pkcs1_15.new(server_std_privkey).sign(SHA256.new(snd_response))
                        dns_socket.sendto(snd_response, snd_address)
                    else:
                        dns_socket.sendto(recv_data[0][66:], ("8.8.8.8", 53))
