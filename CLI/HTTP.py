...
def up(arguments):
    ...
    if NIEPPROCESS is not None:
        if NIEPPROCESS.pid not in [p.pid for p in psutil.process_iter()]:
            NIEPPROCESS = None
        else:
    ...
    try:
        NIEPTIME = time.time()
        NIEPPROCESS = subprocess.Popen(
            ["gnome-terminal", "--", sys.executable, "Launcher.py", arguments["path"]]
        )
        NIEPPROCESS.wait()
